...
use ndarray::{Array, Array1, Array2, ArrayBase, Axis, Ix2, OwnedRepr};
...
pub fn run_clustering(
    ...
    let mut centroids_host =
        Array2::<f32>::zeros((num_clusters as usize, vector_dims as usize));
    ...
    let _inertia_pred = kmeans::predict(&res, &kmeans_params, &dataset, &None, &centroids_gpu, &mut labels_gpu, false).expect("kmeans prediction failed");
    info!(
...
pub fn run_clustering_consolidate(
    ...
    let weights_array = Array1::from_shape_vec(
        num_vectors,
        weights.to_vec(),
    )
        .expect("shaping vectors failed");
    ...
    let weights = ManagedTensor::from(&weights_array)
        .to_device(&res).expect("weights(host)->GPU transfer failed");
...
/*    let distance_operator_cuvs = distance_type_from_str(&distance_operator)
    ...
    info!("running kemans");
    let (inertia, n_iter) = kmeans::fit(&res, &kmeans_params, &dataset, &Some(weights), &mut centroids_gpu)
        .expect("kmeans training failed");
    info!("kmeans done with inertia: {inertia}, n_iter: {n_iter}");
    info!(
        "⏱️ kmeans training data done at: {:.2?}",
