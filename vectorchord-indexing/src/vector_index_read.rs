...
impl VectorReadBatcher {
    pub fn new(
        ...
        // --- THE SNAPSHOT FIX ---
        // Since reltuples is -1, SPI is "blind." We push a fresh snapshot to see the data.
        unsafe {
        ...
        // 1. GET ACCURATE COUNT (Bypass catalog -1)
        ...
        // 2. CALCULATE VECTORCHORD OFFSET
        ...
        info!("📊 [VectorChord] Verified Rows: {} | Random Offset: {}", total, offset);
        ...
        let (cached_vectors, dims) = Spi::connect(|client| {
            ...
            // Note: We use the table name exactly as provided since psql verified it
            let query = format!(
                "SELECT {} FROM {} OFFSET {} LIMIT {}",
                column_name, table_name, offset, num_samples
            );

            let table = client.select(&query, None, &[]).expect("SPI Data Load Failed");

            for row in table {
                let datum = row.get_datum_by_ordinal(1).expect("Column missing").value::<Datum>();
        ...
        if count_loaded == 0 {
             pgrx::error!("FATAL: Table found ({} MB) but SPI read 0 rows. Transaction isolation issue.", 4976);
        }

        info!("✅ VectorChord Ready: Loaded {} vectors in {:.2?}", count_loaded, start_time.elapsed());
