...
impl VectorReadBatcher {
    pub fn new(
        ...
        let (cached_vectors, dims) = Spi::connect(|client| {
            ...
            for row in table {
                ...
                if let Ok(Some(d)) = datum {
                    let byte_slice = unsafe { pgrx::varlena_to_byte_slice(d.cast_mut_ptr()) };
        ...
        info!("✅ Load Complete: {} vectors in {:.2?}", cached_vectors.len() / (safe_dims as usize), start_time.elapsed());
